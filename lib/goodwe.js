 ...
 *
 * BELANGRIJK: elke fase krijgt ALTIJD expliciet alle waarden (Power, Current,
 * Voltage, Energy/Forward). De ongebruikte fasen krijgen 0 W / 0 A maar wél een
 * geldige spanning. Zonder een geldige waarde op de andere fasen beschouwt
 * Victron die niet als echte fasen en plaatst het het vermogen alsnog op L1.
 *
 ...
 * @param {number} [opts.nominalVoltage=230]  fallback-spanning als de meting ontbreekt
 * @returns {object}
 */
function toVictronPayload(data, opts = {}) {
  ...
  // Geldige spanning voor alle fasen: de gemeten netspanning, of de fallback.
  const voltage = data.ac.l1.voltage != null ? data.ac.l1.voltage : nominalVoltage;
  const current = data.ac.l1.current != null ? data.ac.l1.current : 0;
  ...
  for (let i = 1; i <= nrOfPhases; i++) {
    ...
    // Alle fasen krijgen expliciet álle waarden, zodat Victron ze als echte
    // fasen herkent en het vermogen op de juiste fase plaatst.
    payload[`${pre}/Voltage`] = voltage;
    payload[`${pre}/Current`] = active ? current : 0;
    payload[`${pre}/Power`] = active ? data.ac.power : 0;
    payload[`${pre}/Energy/Forward`] = active ? data.energy.total : 0;
  }
  return payload;
}
