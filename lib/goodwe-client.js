 ...
 *
 * BELANGRIJK: de GoodWe/Solarman-dongle stuurt zijn antwoord terug op poort
 * 48899 (vaak als broadcast), NIET naar de efemere bronpoort. Daarom moet de
 * socket op poort 48899 gebonden zijn om het antwoord op te vangen.
 *
 * @param {object} [opts]
 * @param {string} [opts.broadcastAddress='255.255.255.255']
 * @param {number} [opts.targetPort=48899]  doelpoort van het verzoek
 * @param {number} [opts.localPort=48899]   lokale poort om op te luisteren (0 = efemeer, voor tests)
...
function discover(opts = {}) {
  ...
  const targetPort = opts.targetPort || DISCOVERY_PORT;
  const localPort = opts.localPort != null ? opts.localPort : DISCOVERY_PORT;
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.bind(localPort, () => {
      ...
      socket.send(DISCOVERY_REQUEST, targetPort, broadcastAddress, (err) => {
        if (err) {
          if (timer) clearTimeout(timer);
          try {
            socket.close();
